...
class StudentDetails extends StatefulWidget {
  ...
  @override
  State<StudentDetails> createState() => _StaffDetailsState();
}

class _StaffDetailsState extends State<StudentDetails> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: _firestore.collection('students').where("id", isEqualTo: widget.id).snapshots(),
                    ...
                    child: Column(
                      children: [
                        ...
                        InputField(
                          ...
                          hint: '',
                          readOnly: true,
                          controller: TextEditingController(text: document['gender'] == 'male' ? 'Male' : 'Female'),
                        ),
                      ],
                    ),
                  ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          try {
            QuerySnapshot document = await _firestore.collection('students').where("id", isEqualTo: widget.id).limit(1).get();
